    ...
    public class RegisterUserCommandValidator : AbstractValidator<RegisterUserCommand>
    {
        public RegisterUserCommandValidator()
        {
            ...
            RuleFor(x => x.Password)
                ...
                .Matches("[A-Z]").WithMessage("{Password} debe contener al menos 1 mayúscula.")
                .Matches("[a-z]").WithMessage("{Password} debe contener al menos 1 minúscula.")
                .Matches("[0-9]").WithMessage("{Password} debe contener al menos 1 número.")
                .Matches("[^a-zA-Z0-9]").WithMessage("{Password} debe contener al menos 1 caracter especial.");

            RuleFor(x => x.NumberId)
                .NotEmpty().WithMessage("{NumberId} no puede estar en blanco")
                .NotNull();
            ...
            RuleFor(x => x.Role)
                .NotEmpty().WithMessage("{Role} no puede estar en blanco")
                .NotNull();

            RuleFor(x => x.UserName)
                .NotEmpty().WithMessage("{UserName} no puede estar en blanco")
                .NotNull();
        }
    }
}
